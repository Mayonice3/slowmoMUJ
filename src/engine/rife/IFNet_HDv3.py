...
class IFBlock(nn.Module):
    ...
    def forward(self, x, flow=None, scale=1):
        ...
        tmp = F.interpolate(tmp, size=target_size, mode="bilinear", align_corners=False)
        flow = tmp[:, :4] * scale
        mask = tmp[:, 4:5]
        feat = tmp[:, 5:]
...
class IFNet(nn.Module):
    ...
    def forward(self, x, timestep=0.5, scale_list=[8, 4, 2, 1, 1], training=False, fastmode=True, ensemble=False):
        if training == False:
            channel = x.shape[1] // 2
            img0 = x[:, :channel]
            img1 = x[:, channel:]
